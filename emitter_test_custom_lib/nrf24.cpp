...
void init_nrf24(void)
{
    ...
    */
    ...
    /* Cleart TX bit */
    write_register(REG_STATUS, 1 << BIT_TX_DS);
...
void nrf24_set_tx_address(uint8_t *address, uint8_t length)
{
    ...
    */
    ...
    SPI.transfer(W_REGISTER | (REGISTER_MASK & REG_TX_ADDR));
...
{
    /**
    * Read received data which is in FIFO
    *
    * @param data (uint8_t*)    pointer to the buffer to which the fifo data should
    *                           be copied
    * @return none
    */
...
void nrf24_wait_for_ack(uint8_t *payload)
{
    ...
    * If ack packet contiains a payload it is copied in the payload array
    *
    * @param payload (uint8_t*) an array of byte to copy the ack payload to
    * @return none
    */

    uint8_t status = read_register(REG_STATUS);
    while( !(status & (1 << BIT_TX_DS)) ){
      delay(1000);
      status = read_register(REG_STATUS);
      
      if((1 << BIT_MAX_RT) & status) {
        Serial.println("FATAL ERROR : Max retransmit packet, receiver not set up correctly");
      }
...
{
    /**
    * Send data wirelessly using the nrf24
    *
    * @param data (uint8_t*) the data to be sent
    * @param len (uint8_t) number of bytes to be sent
    * @return none
    */

    int i;
    uint8_t r_status = read_register(REG_STATUS);

    /* Wait for the nrf to be ready to send data */
    while( (r_status & ((1 << BIT_TX_DS)  | (1 << BIT_MAX_RT))) ) {
      r_status = read_register(REG_STATUS);
      delay(1000);
      Serial.print("Status : ");
      Serial.println(r_status , BIN);
    }
    Serial.println("");
...
void nrf24_power_tx(void)
{
    ...
    */
    ...
    /* Set RX mode and CRC to 2 bytes (and poweron on) */
    nrf24_set_config(CONFIG_VALUE_TX);
}
